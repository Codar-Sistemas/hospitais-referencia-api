...
async function listHospitals({ stateCode, city, treatment: rawTreatment, q, limit, offset }) {
  if (!stateCode && !city && !q) {
    throw new ValidationError(
  ...
  return {
    filters: { state_code: stateCode, city, treatment: rawTreatment, q, limit, offset },
...
async function getHospital(id) {
  ...
  const row = await hospitalRepo.findById(n);
