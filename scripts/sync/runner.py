...
CLI usage:
  python -m scripts.sync                     # sync all states
  python -m scripts.sync SP                  # sync only SP
  python -m scripts.sync --force SP          # force re-sync
  python -m scripts.sync geocode             # geocode pending rows
  python -m scripts.sync geocode SP          # geocode pending rows in SP
...
from scripts.sync.change_detector import needs_update, pdf_hash_changed
from scripts.sync.scraper import download_pdf, fetch_page_metadata, is_image_based_pdf
from scripts.sync.upserter import upsert_hospitals
...
def sync_state_safe(
    ...
    from scripts.sync.sync_log_writer import write_sync_log
...
def main() -> None:
    ...
    # `python -m scripts.sync SP --force` still works.
